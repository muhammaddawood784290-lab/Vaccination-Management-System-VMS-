...
@section('content')
<div class="space-y-6">
    <div><h2 class="text-lg font-semibold text-gray-900">Vaccination Records</h2><p class="text-sm text-gray-500">All vaccination records at your hospital</p></div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gray-50"><tr>
                    ...
                    <th class="px-5 py-3 text-left text-xs font-medium text-gray-500">Administered By</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($records as $record)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-5 py-3 text-sm text-gray-600">{{ $record->administered_by ?? 'N/A' }}</td>
                            <td class="px-5 py-3 text-sm text-gray-600">{{ $record->administered_at?->format('M d, Y') ?? 'N/A' }}</td>
                            <td class="px-5 py-3"><a href="{{ route('hospital.vaccinations.show', $record) }}" class="text-xs text-blue-600 hover:underline">View</a></td>
                        </tr>
                    @empty
